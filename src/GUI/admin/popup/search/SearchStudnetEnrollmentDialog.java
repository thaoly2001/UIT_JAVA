 ...
 * @author ADMIN
 */
public class SearchStudnetEnrollmentDialog extends javax.swing.JDialog {

    private StudentsDAO dao = StudentsDAO.getInstance();
    private Classes cl;
    private CacheData<Enrollment> enr;
    public SearchStudnetEnrollmentDialog(java.awt.Frame parent, boolean modal, Classes cl, CacheData<Enrollment> enr) {
        super(parent, modal);
        this.cl = cl;
        this.enr = enr;
        initComponents();
    ...
    private void stuTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_stuTableMouseClicked
        if (evt.getClickCount() == 2 && !evt.isConsumed()) {
            ...
            if (row != -1) {
                ...
                e.setClasses(cl);
                e.setStudent(StudentsDAO.getInstance().findById(id));
                enr.put(id, e, EntityState.NEW);
                this.dispose();
            }
        }
